...
######################################################################
#
# PGML.pm - Pre-loadable PGML module
#
...
sub Eval {_call('PG_restricted_eval', @_)}

sub Sort {return _call('lex_sort', @_)}
...
# Called after formatting is complete.
#
sub cleanup {
  my $self = shift;
  # Recursively break {prev} references in the parse tree
  _cleanup_block($self->{root}) if $self->{root};
  # Clear parser's references to the tree
  $self->{root} = undef;
  $self->{block} = undef;
  $self->{split} = undef;
  $self->{string} = undef;
...
sub _cleanup_block {
  ...
  if ($block->{stack}) {
    for my $item (@{$block->{stack}}) {
      $count += _cleanup_block($item) if ref($item);
    }
  }
  return $count;
}
...
sub Math {
  ...
  if ($item->{parsed}) {
    # Use dynamic namespace for context access
    my $context = PGML::_get_hash('context', 'Typeset');
    $context = PGML::_get_hash('context', 'current') if $item->{hasStar};
...
sub Answer {
  ...
  if (defined($ans)) {
	...
	$rule = PGML::_call('NAMED_ANS_RULE', $item->{name}, $item->{width});
	PGML::_call('NAMED_ANS', $item->{name} => $ans);
      } else {
	$rule = PGML::_call('ans_rule', $item->{width});
	PGML::_call('ANS', $ans);
      }
    } else {
	  ...
	  $ans = PGML::_call('String', "");  ### use something else?
        }
      }
      ...
      if (!(ref($ans) eq 'MultiAnswer' && $ans->{part} > 1)) {
        if (defined($item->{name})) {
          PGML::_call('NAMED_ANS', $item->{name} => $ans->cmp);
        } else {
          PGML::_call('ANS', $ans->cmp);
        }
      }
    }
  } else {
...
sub Math {
  my $self = shift;
  return PGML::_call('general_math_ev3', $self->SUPER::Math(@_));
}
...
sub Format {
  ...
  if ($displayMode eq 'TeX') {
    ...
    $format = _call('PTX_cleanup', $format);
  } else {
    $format = '<div class="PGML">'."\n".PGML::Format::html->new($parser)->format.'</div>'."\n";
  }
  _call('WARN_MESSAGE', "==================","Errors parsing PGML:",@warnings,"==================") if scalar(@warnings);
  # Break circular references to allow garbage collection
  $parser->cleanup();
  return $format;
}
...
our $installed = 1;  # Flag to indicate PGML.pm is loaded

...
sub SOLUTION {
	...
	if ($displayMode =~/HTML/ and $envir->{use_knowls_for_solutions}) {
		...
		TEXT(apply_text_filter('solution', $PAR.SOLUTION_HEADING().$BR.$solution_body.$PAR)) if $solution_body ;
	} elsif ($displayMode =~ /PTX/) {
		TEXT('<solution>', "\n", $solution_body, "\n", '</solution>', "\n\n");
	} else {
		TEXT(apply_text_filter('solution', $PAR.$solution_body.$PAR)) if $solution_body ;
	}
}
...
sub HINT {
	...
	if ($displayMode =~/HTML/ and $envir->{use_knowls_for_hints}) {
		...
		TEXT($PAR, apply_text_filter('hint', HINT_HEADING() . $BR. $hint_body) . $PAR);
	}
}
